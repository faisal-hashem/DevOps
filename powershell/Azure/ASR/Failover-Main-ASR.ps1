#Must create the recovery plan name as the same name as the container name. 

#Failover includes Recovery Plan ONLY, use RPI instead?
# Select Subscription --> Select Resource Group for VM --> Select ASR Recovery Vault --> Select ASR Container --> Select ASR Recovery Plan --> Select Storage Account --> Select VNET
...
function Get-ASRRecoveryObjects {  
    ...
    $script:ASRRecoveryPlanName = $ASRRecoveryPlan.Name
    $script:ASRRecoveryPlanMain = Get-AzRecoveryServicesAsrRecoveryPlan -Name $ASRRecoveryPlanName
...
function ASRFailover {
    
    #Starts MAIN Failover job. Inputting Recovery Plan & Secondary Region VNET
    $FOJob = Start-AzRecoveryServicesAsrUnplannedFailoverJob -RecoveryPlan $ASRRecoveryPlanMain -Direction PrimaryToRecovery -Verbose

    Write-Host -ForegroundColor White -BackgroundColor Black "----------------Monitoring-Failover-----------------"
    Write-Host -ForegroundColor White -BackgroundColor Black "----------Status will refresh every 5 seconds------------"

    #do/while is being used to make sure Failover State is provided as long as State is in Progress OR Not-Started
    do {
        $FailoverState = Get-AzRecoveryServicesAsrJob -Job $FOJob | Select-Object State
        ...
        try {
            ...
            Write-Host " "
            Write-Host -ForegroundColor Red -BackgroundColor Black "ERROR -Unable to get status of Failover Job"
        ...
        Write-Host "Failover status for $($FOJob.TargetObjectName) is $($FailoverState.state)"
        Start-Sleep 5;
    }
    while (($FailoverState.state -eq "InProgress") -or ($FailoverState.state -eq "NotStarted"))

    #if/else wll exit code if Failover state is in Failed state. If it is not in failed state it will continue to Clean Up Failover items created. 
    if ($FailoverState.state -eq "Failed") {

        Write-Host("The Failover Job has failed. Scipt will Terminate now")
        exit
    }

    else {
        Read-Host -Prompt "Failover has completed successfully. Please check ASR Portal and test the VMs. Once done, please press ENTER here to continue the Script to Re-Protect failed over VMs"

        #Starting to Clean Up all resources created by Failover job.
        $FOJobReProtect = foreach ($RPI in $PrimaryRPI){Update-AzRecoveryServicesAsrProtectionDirection -ReplicationProtectedItem $RPI -AzureToAzure -ProtectionContainerMapping $RecoveryContainerMapping -LogStorageAccountId $RecoveryCacheStorageAccount.Id -RecoveryResourceGroupId $PrimaryResourceGroup.ResourceId} -Verbose
        ...
        do {
            $FOCleanUpState = Get-AzRecoveryServicesAsrJob -Job $FOJobReProtect | Select-Object State
            ...
            Write-Host "Cleanup Status for $($FOJob.TargetObjectName) is $($FOCleanUpState.state)"
            Start-Sleep 5;
        }
        while (($FOCleanUpState.state -eq "InProgress") -or ($FOCleanUpState.state -eq "NotStarted"))


        Write-Host "Failover Cleanup Completed"
    }
}  
...
function ASRFailback {
    ...
    if ($FailbackState.state -eq "Failed") {
        ...
        $FBJobReProtect = foreach ($RRPI in $RecoveryRPI){Update-AzRecoveryServicesAsrProtectionDirection -ReplicationProtectedItem $RRPI -AzureToAzure -ProtectionContainerMapping $PrimaryContainerMapping -LogStorageAccountId $PrimaryCacheStorageAccount.Id -RecoveryResourceGroupId $RecoveryResourceGroup.ResourceId} -Verbose
