...
Foreach ($user in $licensedIT){
    $currDisabled = $user.Licenses | where {$_.AccountSKUID -eq "generalatlantic:SPE_E3"} | select -ExpandProperty ServiceStatus | where {$_.ProvisioningStatus -eq "Disabled"} | select -ExpandProperty ServicePlan | select -ExpandProperty ServiceName
...
foreach ($account in $peopleToFix){    
    ...
    $currDisabled = $msolUser.Licenses | where {$_.AccountSkuID -eq "generalatlantic:SPE_E3"} | select -ExpandProperty ServiceStatus | where {$_.ProvisioningStatus -eq "Disabled"} | select -ExpandProperty ServicePlan | select -ExpandProperty ServiceName
    ...
    $licObj = New-MsolLicenseOptions -AccountSkuId "generalatlantic:SPE_E3" -disabledplans $newDisabled
