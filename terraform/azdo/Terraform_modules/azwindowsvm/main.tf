locals {
  ...
  domain_name = "na.gapartners.com"
  domain_oupath = "OU=${var.region},OU=GA-Servers,DC=na,DC=gapartners,DC=com"
  adjoin_user = "gap\\SVCSCCMNet"
...
resource "azurerm_virtual_machine_extension" "gadomainjoin" {
...
resource "azurerm_virtual_machine_extension" "garootdomainjoin" {
  count = var.enable_rootdomainjoin ? 1 : 0
  depends_on = [azurerm_windows_virtual_machine.vm]
  name                 = "garootdomainjoin"
  virtual_machine_id   = azurerm_windows_virtual_machine.vm.id
  publisher            = "Microsoft.Compute"
  type                 = "JsonADDomainExtension"
  type_handler_version = "1.3"
  settings = jsonencode(
    {
"Name": "gapartners.com",
#"OUPath": "OU=${var.region},OU=GA-Servers,DC=gapartners,DC=com",
"OUPath": "CN=Computers,DC=gapartners,DC=com",  #-- We currently have no machines in the root EXCEPT for Domain Controllers, so they're all moved to 'Domain Controllers' once promoted.
"User": "gap\\SVCSCCMNet",
"Restart": "true",
"Options": "3"
    }
...
resource "azurerm_virtual_machine_extension" "pdqdriveformatting" {
  ...
  depends_on = [
    azurerm_virtual_machine_extension.gadomainjoin, 
  ...
  protected_settings = <<PROTECTED_SETTINGS
    {
      ...
      "StorageAccountName": "gaproddeploy",
      "storageAccountKey": "${var.deploystorageaccountkey}",
      "fileUris": [
        "https://gaproddeploy.blob.core.windows.net/powershell/InvokeDriveConfig.ps1"
      ]
...
resource "azurerm_virtual_machine_extension" "setnewbuildregkey" {  # If the machine does not require additional disks, it will still get the new build key for standard PDQ packages
    ...
    settings = <<SETTINGS
    {
        "commandToExecute" : "powershell -command New-Item HKLM:/Software/GA -Force ; New-ItemProperty -Path HKLM:/Software/GA -Name NewBuild -Value 1 -PropertyType DWORD ; exit 0"
...
resource "azurerm_virtual_machine_extension" "bginfo" {
  ...
  settings = jsonencode({
      "configurationUrl": "https://gaproddeploy.blob.core.windows.net/bginfo/Bginfo.bgi"
  })
  protected_settings = jsonencode({
      "storageAccountName": "gaproddeploy",
      "storageAccountKey": "${var.deploystorageaccountkey}",
...
#       "storageAccountKey": "${var.deploystorageaccountkey}",
#       "fileUris": [
#                 "https://gaproddeploy.blob.core.windows.net/powershell/azpostdeploy.ps1"
#             ]
#     }
# )
# depends_on = [azurerm_virtual_machine_extension.gadomainjoin]
# }